...
resource "aws_route53_zone" "internal" {
  ...
  name = "internal.${replace(var.projectname, " ", "-")}.dev"
}

resource "aws_route53_zone" "public" {
	count = var.environment == "dev" ? 0:1
	name  = "${replace(var.projectname, " ", "-")}.com"
}

locals {
	 deployment = var.environment == "dev" ? "internal.${replace(var.projectname, " ", "-")}.dev" : "${replace(var.projectname, " ", "-")}.com"
